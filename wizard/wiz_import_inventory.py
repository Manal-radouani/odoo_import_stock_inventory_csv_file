...
class import_inventory(models.TransientModel):
	...
	File_slect = fields.Binary(string="Select File",required=True)
	...
	def imoport_file(self):
		inventory=self.env['stock.inventory'].create({'name':self.inventory_reference,'location_id':self.location_id.id,'state':'confirm'})
		...
		for k,v in lines['real_product_qty'].items():
			if k[0] == "":
				...
				loan_lines.append((0, _, {'location_id': location_id, 'product_id': product_id,
										  'product_qty':v,'product_uom_id':unit.id}))
